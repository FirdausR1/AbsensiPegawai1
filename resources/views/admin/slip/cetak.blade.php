...
<head>
    ...
    <title>Slip Gaji - {{ $pegawai->nama }} ({{ $bulan }})</title>
...
<body class="bg-slate-100 min-h-screen p-4 sm:p-8 text-slate-900 leading-relaxed">
    ...
    <div class="max-w-3xl mx-auto mb-6 flex items-center justify-between no-print">
        <a href="{{ route('admin.slip.index', ['bulan' => $bulan, 'site' => $pegawai->area_kerja]) }}" class="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-800 text-xs font-bold rounded-xl transition">
        ...
        <button onclick="window.print()" class="px-6 py-2.5 bg-[#000d6b] hover:bg-[#001253] text-white text-xs font-bold rounded-xl shadow-md transition flex items-center gap-2">
            ...
            Cetak Slip Gaji (Print / PDF)
        </button>
    </div>

    <!-- Official Paper Frame -->
    <div class="max-w-3xl mx-auto bg-white p-8 sm:p-10 shadow-md border border-slate-200 rounded-2xl space-y-5">
