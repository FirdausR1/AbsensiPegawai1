                    ...
                    @forelse($pegawais as $p)
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="py-4 px-4">
                                ...
                                <div class="text-[10px] text-slate-600 mt-0.5">💳 {{ $p->nama_bank ? $p->nama_bank . ': ' . $p->nomor_rekening : 'Rekening: Belum diisi' }}</div>
                            </td>
...
<div id="modal-gaji" class="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4 hidden overflow-y-auto">
    <div class="bg-white rounded-2xl border border-slate-200 max-w-lg w-full p-6 shadow-xl space-y-4 my-8">
        ...
        <form id="form-gaji" method="POST" action="" class="space-y-4 text-xs">
            ...
            <!-- Setting Rekening Bank (Payroll) -->
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                <div class="font-extrabold text-slate-800 text-xs">💳 Rekening Bank Transfer (Payroll)</div>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <div>
                        <label class="block font-bold text-slate-700 mb-1 text-[11px]">Nama Bank</label>
                        <input type="text" name="nama_bank" id="modal-nama-bank" class="w-full px-3 py-1.5 rounded-lg border border-slate-300 outline-none text-slate-800 text-xs" placeholder="e.g. BCA, Mandiri">
                    </div>
                    <div>
                        <label class="block font-bold text-slate-700 mb-1 text-[11px]">Nomor Rekening</label>
                        <input type="text" name="nomor_rekening" id="modal-nomor-rekening" class="w-full px-3 py-1.5 rounded-lg border border-slate-300 outline-none font-mono text-slate-800 text-xs" placeholder="e.g. 5210987654">
                    </div>
                    <div>
                        <label class="block font-bold text-slate-700 mb-1 text-[11px]">Atas Nama Rekening</label>
                        <input type="text" name="nama_rekening" id="modal-nama-rekening" class="w-full px-3 py-1.5 rounded-lg border border-slate-300 outline-none text-slate-800 text-xs" placeholder="Nama pemilik rek">
                    </div>
                </div>
            </div>
...
<script>
    function openModalEditGaji(pegawai) {
        ...
        document.getElementById('modal-nama-bank').value = pegawai.nama_bank || '';
        document.getElementById('modal-nomor-rekening').value = pegawai.nomor_rekening || '';
        document.getElementById('modal-nama-rekening').value = pegawai.nama_rekening || '';
